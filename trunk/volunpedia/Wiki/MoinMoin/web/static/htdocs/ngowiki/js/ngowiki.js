$( window ).ready(function() {
	...
	if(!!window.__page_meta && (window.__page_meta.tags.length > 0 || window.__page_meta.locations.length > 0)) {
		...
		if(t.length >= 1) {
			...
			node.innerHTML = '标签:' + '<span style="padding-right:5px;">&nbsp;</span>' + [].concat(window.__page_meta.tags).concat(window.__page_meta.locations).join(", ")
	...
	if(!!window.__ListPagesByTag_filterByTag && !!document.getElementById("listpagesbytag_filter")) {
	    ...
	    var html = "筛选：<span id='currentTags' style='margin-right:20px;display:none;'></span><a href='javascript:displayRelatedTagsPanel()'>添加</a><div id='relatedTagsPanel' style='display:none'></div>";
		node.innerHTML = html;
